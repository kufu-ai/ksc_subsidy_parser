...
def merge_classification_individual_results(original_results, extracted_results):
    ...
    # 個別ページのみを抽出（新スキーマのみ）
    original_individual = [
        r for r in original_results if r.get("page_type") == "住宅関連個別ページ"
    ]
    extracted_individual = [
    ...
    return {
        ...
        "statistics": {
            ...
            "duplicate_removed": len(original_individual)
            + len(extracted_individual)
            - len(merged_individual),
        },
    }
...
def create_comprehensive_summary(merged_data):
    ...
    return {
        ...
        "prefecture_stats": dict(
            sorted(prefecture_stats.items(), key=lambda x: x[1], reverse=True)
        ),
        "city_stats": dict(
...
def save_merged_results(merged_data, comprehensive_summary, base_filename):
    ...
    for page in individual_pages:
        ...
        if page.get("found_new_housing_subsidies"):
            ...
            urls.append(
                {"url": page.get("url", ""), "title": page.get("page_title", "")}
            )
        # 既存の市区町村エントリを探す
...
def main():
    ...
    try:
        ...
        if extraction_file.exists():
            ...
            print(
                "⚠️  対応する抽出結果ファイルが見つかりません。元の結果のみを処理します。"
            )
            extracted_results = []

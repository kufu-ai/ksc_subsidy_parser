...
def merge_classification_results(original_results, extracted_results):
    ...
    """
    # 個別ページのみを抽出（新スキーマのみ）
    original_individual = [r for r in original_results if r.get('page_type') == '住宅関連個別ページ']
    extracted_individual = [r for r in extracted_results if r.get('page_type') == '住宅関連個別ページ']
